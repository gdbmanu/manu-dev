...
class Grid_AttentionTransNet(nn.Module):
    ...
    def __init__(self, do_stn=True, do_what=False, LAMBDA=.1, deterministic=False):
        ...
        ##  The where pathway        
        self.num_features = self.vgg_where.classifier[0].in_features
        #features = list(self.vgg_where.classifier.children())[:-1] # Remove last layer
        #features.extend([nn.Linear(num_features, 500)]) # Add our layer
        self.vgg_where.classifier = nn.Identity() #Sequential(*features) # Replace the model classifier
...
for epoch in range(args.epochs):
    ...
    torch.save(model, f"out/230311_logPolarGrid_vgg_stn_{LAMBDA}_{args.radius}.pt")
    with open(f"out/230311_logPolarGrid_vgg_stn_{LAMBDA}_{args.radius}.pkl", "wb") as f:
        train_data = {
                "train_acc" : train_acc,
