...
for epoch in range(args.epochs):
    ...
    torch.save(model, f"221231_logPolarGrid_vgg_stn_{deterministic}_{LAMBDA}_shrink.pt")
    np.save(f"221231_logPolarGrid_vgg_stn_{deterministic}_{LAMBDA}_shrink_acc", acc)
    np.save(f"221231_logPolarGrid_vgg_stn_{deterministic}_{LAMBDA}_shrink_loss", loss)
    np.save(f"221231_logPolarGrid_vgg_stn_{deterministic}_{LAMBDA}_shrink_kl_loss", kl_loss)
