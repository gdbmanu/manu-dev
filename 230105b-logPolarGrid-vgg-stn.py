...
for LAMBDA in (0.1,):
    ...
    for epoch in range(args.epochs):
        ...
        torch.save(model, f"230105b_logPolarGrid_vgg_stn_{deterministic}_{LAMBDA}_{radius}.pt")
        np.save(f"230105b_logPolarGrid_vgg_stn_{deterministic}_{LAMBDA}_{radius}_acc", acc)
        np.save(f"230105b_logPolarGrid_vgg_stn_{deterministic}_{LAMBDA}_{radius}_loss", loss)
        np.save(f"230105b_logPolarGrid_vgg_stn_{deterministic}_{LAMBDA}_{radius}_kl_loss", kl_loss)
