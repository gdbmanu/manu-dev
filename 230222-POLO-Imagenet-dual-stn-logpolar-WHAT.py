...
def train(epoch, loader, n_sample_train):
    ...
    if n_sample_train is None:
        n_sample_train = len(loader) // args.batch_size
...
def test(loader, n_sample_test):
    with torch.no_grad():
        ...
        if n_sample_test is None:
            n_sample_test = len(loader) // args.batch_size
...
if __name__ == '__main__':
    ...
    for epoch in range(args.epochs):
        ...
        torch.save(model, f"out/230222_polo_stn_dual_WHAT_{args.radius}.pt")
        with open(f"out/230222_polo_stn_dual_WHAT_{args.radius}.pkl", "wb") as f:
            train_data = {
                "train_acc" : train_acc,
