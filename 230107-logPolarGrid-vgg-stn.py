...
for epoch in range(args.epochs):
    ...
    torch.save(model, f"230107_logPolarGrid_vgg_stn_WHAT.pt")
    np.save(f"230107_logPolarGrid_vgg_stn_WHAT_acc", acc)
    np.save(f"230107_logPolarGrid_vgg_stn_WHAT_loss", loss)

for LAMBDA in (1, 0.1, 0.01, 0.001, 1e-4, 1e-5):
    acc = []
    loss = []
    kl_loss = []
    model = Grid_AttentionTransNet(do_stn=do_stn, LAMBDA=LAMBDA, deterministic=deterministic).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        torch.save(model, f"230107_logPolarGrid_vgg_stn_{deterministic}_{LAMBDA}_{radius}.pt")
        np.save(f"230107_logPolarGrid_vgg_stn_{deterministic}_{LAMBDA}_{radius}_acc", acc)
        np.save(f"230107_logPolarGrid_vgg_stn_{deterministic}_{LAMBDA}_{radius}_loss", loss)
        np.save(f"230107_logPolarGrid_vgg_stn_{deterministic}_{LAMBDA}_{radius}_kl_loss", kl_loss)
