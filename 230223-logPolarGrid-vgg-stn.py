...
model = Grid_AttentionTransNet(do_stn=True, LAMBDA=LAMBDA, deterministic=deterministic).to(device)
optimizer = optim.Adam(model.fc_what.parameters(), lr=lr)

#model = torch.load(f"230107_logPolarGrid_vgg_stn_WHAT.pt")
model = torch.load("../JNJER/230105_logPolarGrid_vgg_stn_WHAT.pt", map_location=device)
...
for epoch in range(args.epochs):
    ...
    torch.save(model, f"230223_logPolarGrid_vgg_stn_{LAMBDA}_{args.radius}_{deterministic}.pt")
    with open(f"230223_logPolarGrid_vgg_stn_{LAMBDA}_{args.radius}_{deterministic}.pkl", "wb") as f:
        train_data = {
                "train_acc" : train_acc,
