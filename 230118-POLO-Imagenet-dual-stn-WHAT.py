...
from POLO.PYramid2 import cropped_pyramid, get_K, log_gabor_transform
...
if __name__ == '__main__':
    ...
    model = Polo_AttentionTransNet(do_stn = args.do_stn, do_what=args.do_what, LAMBDA=LAMBDA).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        torch.save(model, f"230118_polo_stn_dual_WHAT.pt")
        np.save(f"230118_polo_stn_dual_WHAT_acc", acc)
        np.save(f"230118_polo_stn_dual_WHAT_loss", loss)
        np.save(f"230118_polo_stn_dual_WHAT_kl_loss", kl_loss)
