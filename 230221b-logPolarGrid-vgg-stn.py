...
test_kl_loss = []
test_entropy = []
with open(f"out/230221_logPolarGrid_vgg_stn_0_1.pkl", "rb") as f:
    data = pickle.load(f)
    data = edict(data)
...
model = torch.load("out/230221_logPolarGrid_vgg_stn_0_1.pt")
model.LAMBDA = LAMBDA
...
for epoch in range(args.epochs):
    ...
    torch.save(model, f"out/230221b_logPolarGrid_vgg_stn_{LAMBDA}_{args.radius}.pt")
    with open(f"out/230221b_logPolarGrid_vgg_stn_{LAMBDA}_{args.radius}.pkl", "wb") as f:
        train_data = {
                "train_acc" : train_acc,
